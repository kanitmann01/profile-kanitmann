...
describe("ExperienceTimeline", () => {
  ...
  it("renders period for visible experiences", () => {
    ...
    visibleExps.forEach((exp) => {
      if (exp.startDate && exp.endDate) {
        ...
        // Multiple concurrent roles can share the same period; require at least one match.
        expect(screen.getAllByText(periodRegex).length).toBeGreaterThan(0);
      }
      if (exp.roles) {
        exp.roles.forEach((role) => {
  ...
  it("renders the vertical connector line", () => {
    ...
    const connectorLine = container.querySelector(
