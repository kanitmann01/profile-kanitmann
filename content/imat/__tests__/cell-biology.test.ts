...
describe("cell-biology topic", () => {
  it("has a topic entry in the registry", () => {
    const topic = topics.find(
      (t) => t.slug === "cell-biology" && t.subject === "biology"
    );
  ...
  it("is listed in the biology subject's topics", () => {
    ...
    expect(bio?.topics).toContain("cell-biology");
  });

  it("has all 9 notes registered", () => {
    for (const slug of EXPECTED_SLUGS) {
      const note = notes.find((n) => n.slug === slug);
      expect(note, `missing note: ${slug}`).toBeDefined();
      expect(note?.subject).toBe("biology");
      expect(note?.topic).toBe("cell-biology");
    }
  });
